...
export async function run(source : string, config: any) : Promise<number> {
  ...
  const tcProg = typeCheckProgram(parsed);
  var returnType = "";
  var returnExpr = "";
  const lastExpr = tcProg.stmts[tcProg.stmts.length - 1]
  
  if(lastExpr && lastExpr.tag === "expr") {
    returnType = "(result i32)";
    returnExpr = "(local.get $$last)"
  }
  const compiled = compiler.compile(tcProg);
